...
NetworkList = ["simsiam_recheck"]; %["LFNet", "singlehdr", "maskhdr", "hdrgan", "hdrcnn", "expandnet", "drtmo"];
...
for netIdx = 1:length(NetworkList)
    ...
    hdrFileFlag = false;
    ext = ".exr";
    if NetworkList(netIdx) == "simsiam_recheck"
        ext = ".hdr";
        hdrFileFlag = true;
    end
    ...
    reconFullPath = hdrBasePath + reconPath + "*" + ext;
    ...
    for idx = 1:length(reconFolderInfo)
        ...
        if hdrFileFlag == true %NetworkList(netIdx) == "LFNet"
