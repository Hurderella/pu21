function BRISQUE_TEST%(input)
    ...
    NetworkList = ["TEST_RESULT_2025_03_04__12_12_41"];
...
function ret = getBRISQUE_PIQE(reconFilePath, referenceFilePath, hdrFileFlag, Lpeak)
    ...
    mm = max(reconFile(:));
    reconFile = reconFile / max(reconFile(:)) * Lpeak + epsilon;
    referenceFile = referenceFile / max(referenceFile(:)) * Lpeak + epsilon;
